# 🚀 Proyecto de Transformación Digital (RA6)

## 🧑‍💼 1. Empresa
- Nombre de la empresa: Mercadona
- Sector: Supermercado
- Descripción breve: Empresa de España con tiendas físicas que ha ido incorporando progresivamente herramientas digitales (tienda online, 	logistica avanzada, tickets digitales..) 

---

## 🎯 2. Objetivos estratégicos (SMART)

### Objetivo 1
- Descripción: Mejorar la eficiencia de la logistica y distribución 
- Métrica: Reducir en un 20% el tiempo medio de reposición de productos y disminuir las roturas de stock en un 15%
- Plazo: 1 año 

### Objetivo 2
- Descripción: Optimizar la experiencia del cliente en el ambito online
- Métrica: Aumentar en un 25% las ventas online y mejorar la satisfacción del cliente
- Plazo: 1 año 

### Objetivo 3
- Descripción: Automatizar proyectos internos mediante datos
- Métrica: Reducir en un 30% los errores en la gestión de inventario y pedidos
- Plazo: 1 año 

---

## 🏢 3. Áreas de la empresa

| Área | Función | Sub-objetivo digital | Impacto esperado |
|------|--------|----------------------|------------------|
|   Tiendas Físicas y Online   |  Venta de productos y atencion al cliente       | Implementar sistemas digitales de compra y pago | Mejorar la experiencia del cliente
| Logística      |  Gestion de almacenes      | Automatizar la gestión de inventario |  Reduccion de costes    |
|  Maretig     |      Gestion de almacenes y stock   |          Uso de big data             |Mayor fidelizacion y aumento de ventas             |

---

## ⚙️ 4. Tecnologías habilitadoras digitales (THD)

- Tecnología 1: Big Data 
  - Uso en la empresa: Analizar compras
- Tecnología 2: Cloud Computing 
  - Uso en la empresa: Gestion de sistemas
- Tecnología 3: IA
  - Uso en la empresa: Prediccion de demanda 
- Tecnología 4: Apps moviles 
  - Uso en la empresa: Para el uso por clientes 
- Tecnología 5:
  - Uso en la empresa:

---

## 📊 5. Datos y tratamiento de la información

- Datos que maneja:
   Compras de clientes
   Stock de productos
   Datos de proveedores
- Información que se obtiene:
   Productos más vendidos
   Tendencias de consumo
   Optimización de inventario

---

## 🔐 6. Seguridad y riesgos

### Riesgos identificados
1. Robo de datos de clientes
2. Ciberataques
3. Errores de guardado de datos

### Medidas de protección
1. Cifrado de datos
2. Sistema de seguridad y control de acceso 
3. Inversion en bases de datos

---

## 👨‍💻 7. Recursos humanos

- Nuevos perfiles necesarios: Analisis de datos
- Formación requerida: Uso de herramientas
- Cambios organizativos: Formacion digital continua, nuevos perfiles tecnologicos. 

---

## 🔗 8. Integración de sistemas

- ERP utilizado o propuesto: Tienda online, almacenes, 
- CRM utilizado o propuesto: Sistemas de ventas
- BPM / automatización: Reposicion automatica de stock 
- Integración entre sistemas: Pedidos gestionados automatica

---

## 💡 9. Valor del proyecto

- Beneficios económicos: Mayor eficiencia, mejores decisiones basadas en datos, mejor experiencia del cliente
- Mejora operativa:
- Ventaja competitiva:
- Riesgos de no digitalizar: Menor adaptacion de mercado, perdida de competitividad

---

## 📐 10. Modelado UML (OBLIGATORIO)

### Casos de uso
- Actores: Cliente, empleado, responsable logística, proveedor, administrador
- Funcionalidades principales:Realizar compra online, consultar productos, gestionar stock, reposición automática, generar ticket, gestionar pedidos

### Diagrama de clases
- Entidades principales: 
- Relaciones:

### Diagrama de secuencia (opcional)
- Caso representado: Adjunto la imagen de modelio en el mismo apartado que este pdf, en moodle. 

---
